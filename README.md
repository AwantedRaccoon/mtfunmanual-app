...
> - 工程阶段：Batch 0 已完成；Batch 1 的本地实现与 Simulator 自动化已完成，但整体完成门禁尚未关闭；Batch 2 已在 Simulator 与自动化范围内完成。
> - 当前位置：位于 Batch 3“今日执行、提醒与 Countdown”开始之前。
> - GitHub 状态：Stage 0–2 已整理为本地四提交阶段快照，尚未 push，也不是 GitHub Release。
> - 发布状态：尚未达到 App Store release-ready，也尚未开始发布前真机测试。

## 我们在哪里

目前落地的是一套可迁移、可恢复、可审计的本地数据底座，以及稳定的时间事实和方案版本骨架。用户已经可以记录并封存个人方案变化，但“今天该做什么、实际做了什么”的每日执行闭环尚未实现。
...
- “今天”：HRT 天数、一个基础 Countdown 的记录与显示、快速记录入口；尚无执行确认或本地提醒；
...
尚未实现的主要能力包括：ScheduleEngine、occurrence、执行事实与纠错、本地提醒、Countdown 完整状态机、库存与 lot ledger、完整化验模型、状态与附件、统一时间线、应用锁、最近任务遮挡、删除/重置、Readable JSON v2、安全恢复、PDF/CSV/完整备份、正式药品目录、公共内容包和确定性分析。README 不把这些能力描述成已经完成。

## 最近验证

2026-07-21 的 Stage 0–2 阶段快照基线：

- generic iOS Simulator 无签名 Debug build 通过；
- 单元与渲染测试 `119/119` 通过，UI 测试 `13/13` 通过；
- Release 合同测试 `9/9` 通过，其中包含一轮真实五年 worker 正确性回归；
- Release-config Simulator performance preflight `1/1` 通过，完成 1 次预热与 20 个正式样本；`acceptance = not-evaluated`，数值阈值与跨设备冻结 fixture 尚未确定；
- 渲染矩阵覆盖 320×568、390×844、430×932、768×1024、844×390 横屏，以及 320×568 最大辅助字号场景；
- 每个模块仍须由未参与实现的 reviewer 独立审查；具体当次结论属于交付记录，不把 reviewer 身份或本机证据路径写入公开快照。
...
Batch 3 及以后是当前计划顺序；在对应 ADR 接受前，它们是路线图，不是已经冻结的技术实现。
...
| **Batch 3** | **下一计划批次** | 今日执行、提醒与 Countdown |
| Batch 4 | 计划中 | 库存闭环 |
...
- **GitHub 阶段快照**：许可证、AppIcon 来源、Batch 0 合同以及 Batch 1–2 工程快照已在 `codex/github-stage-0-2-report` 整理为四个本地提交；内部工作日志、构建产物、Simulator 标识和本机路径均不在提交中。该分支尚未 push，等待维护者审阅，不把阶段快照描述为 App Release；
- **App Store**：当前不 ready。除 Batch 3–9 的产品能力外，仍需完成真机、签名 Release Candidate、发行主体/地区、隐私与医疗分类、内容授权等发布门禁；

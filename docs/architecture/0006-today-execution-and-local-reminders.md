...
- 内部全局预算冻结为 60 个 pending request，目标窗口 14 个当地日；预算必须先扣除通知中心中其他来源的 pending request，再先覆盖每条启用规则的下一项，最后按 fire date 和 key 稳定排序填充。每次 add 前都重新回读并按最新 foreign 数量裁剪可提交的 owned 前缀，禁止先超额提交再事后收缩。即使调和开始时总数已经超过 60，也必须先区分 foreign 与 owned：只要删除超出当前容量的 owned 后可以收敛，就保留仍匹配的 authoritative 前缀，不能因初始总数超预算而提前清空全部 owned。调和尾声还需有界地重复回读并按最新 foreign 数量收缩 owned 集合；最终回读以当时的 foreign 数量重新冻结 authoritative owned 前缀。有界收缩能把总数压到 60 时必须保留仍被允许且回读匹配的前缀，并按 schedule / Countdown 各自报告确认数量、`limitedByBudget` 与首个未覆盖时间，不能因为它少于初始计划就清除全部 owned request；一个域的规划错误也不能被另一个域成功收缩时的预算变化错误码覆盖。只有 foreign request 自身超过预算或持续增长，且有界收缩仍无法满足总预算，或回读不一致时，才保守删除全部 owned request、再次回读并报告安排失败。60 是保守产品预算，不宣称为当前 UserNotifications 公布上限。

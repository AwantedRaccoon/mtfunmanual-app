...
V4 → V5 必须在 inactive generation copy 上完成 lightweight migration、幂等 backfill、关系与文件校验、释放并重开校验，最后切换 `5.0.0` pointer。失败时 active pointer 仍指向 V4。若中断发生在 `.preparing` 阶段，恢复必须复用 journal 已冻结的同一个 target generation ID，精确删除其不完整目录后重新准备；不得改换 target UUID 留下 orphan，也不得改写 V4 source 的 durable main/WAL bytes。
...
1. 从 PhotosPicker 或 fileImporter 得到用户明确选择的内容；Photos 使用 `preferredItemEncoding: .current` 和 `FileRepresentation`，避免系统为了兼容性静默转码，并以实际传输文件的 content type 为准，不使用 provider advertised type 列表的第一项猜测 bytes 类型。一次 selection batch 必须拥有独立代次身份；传输期间同时禁用 Picker、开发态 Files 入口与保存，只有当前代次可以清空 selection 和解除门禁。Picker 的系统 selection limit 永远不得以 `0` 表达「已满」（系统会把它解释为不限制），容量已满必须禁用入口并显示明确状态。超出剩余槽位的 selection、旧代次中已确认的失败，以及传输、iCloud 下载、文件面板、类型、大小或总量失败都必须计数并给出可见错误；不能用 `prefix`、`try?`、nil guard 或 stale completion 静默遗漏用户已经选择的附件，更不能继续保存父记录。关闭错误提示不能解除失败门禁；编辑器必须持续显示未解决失败数量，用户重新选择或检查附件后，只有明确确认不再使用未导入附件，才能清除门禁。开发态 Files 的安全作用域、属性核对和最大 60 MiB 有界读取必须在专用串行 I/O queue 上执行，不得阻塞 MainActor；
...
4. 短事务提交 owner、历史时间、attachment metadata、revision 和 operation receipt；化验、状态或普通旅程 owner 与其全部 attachment metadata 必须在同一 SwiftData transaction 中提交；
...
- App ready 后，生产界面只能通过 generation 级 `AttachmentMutationService` 变更附件，不得直接组合 `AttachmentFileStore` 与 `AppDataWriter`。服务把阻塞文件 I/O 放入专用串行 queue，并持有跨 `await` 的全局 mutation lease；actor 重入不能让第二笔导入、删除或预览审计与当前事务交错。`AppDataSession` 与 service 共享同步 Recovery latch，reader、writer 或 service 任一路径使 generation 进入 Recovery 时都先废止该 latch；被旧 Task 持有的 service 必须拒绝后续 mutation/preview。若 Recovery 恰好发生在数据库已提交的文件事务中途，服务仍完成强制 journal/trash 收尾，但不得向 UI 返回成功；
...
Batch 5 保留选择文件的原始 bytes、UTType 与 hash，不静默 OCR、转码、裁切、去 EXIF 或生成被称为「原件」的替代文件。首批预览直接使用受控的原文件 lease；每次交给 Quick Look 前都必须按 metadata 重新执行 opaque path、regular/non-symlink、保护/备份、size 和 hash 审计，失败即进入 Recovery；不持久化 thumbnail。详情页同一时刻只允许一个 pending、presented 或 releasing 的预览请求：Quick Look dismiss 后必须保持全局阻塞状态，直到 `endPreview` 明确完成；在此之前，UI 与 generation service 都拒绝任何附件的后续预览和删除，不能用新的 attachment ID 覆盖尚未释放的 lease。若详情在异步审计完成前退出，旧 completion 必须立即释放已经取得的 preview lease，不能让当前 generation 的删除永久阻塞。删除只承诺从当前 active generation 移除该附件；inactive generation 和系统备份可能仍含历史副本，不能宣称取证级擦除。未来第一次包含附件的 schema generation 升级前，必须另行冻结 inactive generation retention/purge。

隐私清单必须申报项目实际使用的 Required Reason API：App container 文件 metadata 对应 `C617.1`，用户通过系统选择器明确授权的文件 metadata 对应 `3B52.1`。plist 语法和构建资源检查属于 GitHub 阶段门禁；最终签名 Archive 的 privacy report 仍属于发布门禁。
...
- 首屏刷新与分页读取共享单调 request epoch；新刷新必须使旧分页失效，旧 completion 不能追加结果、覆盖错误或清除新请求的 loading 状态；
- 当前 request epoch 的首屏或分页若报告 `corruptionSuspected`，必须清空当前投影、隐藏空状态与继续写入入口并触发当前 generation 的 Recovery；不能把损坏资料显示成「还没有时间线」或普通稍后重试。过期 request 的 completion 仍只允许被忽略；
...
- migration：V4 source 的 durable main/WAL bytes 不变（SQLite `-shm` 是只读连接也可能改变的进程间锁表，不作为事实字节 digest）；`.preparing` 的 copy 中途与 generation prepared 后中断都必须保持 pointer 指向 V4，清理不完整目录并复用同一个 target UUID，且不得留下 orphan；验证后才切 V5；backfill 幂等；逐事实 count/digest/revision/关系/重开校验；legacy 同日/同名不合并；每个 synthetic sample 都有确定性 receipt，删除 sample receipt 或伪造无 legacy 来源的 synthetic 形状必须 fail closed，ledger count/digest/revision 必须同步；
...
- attachments：取消与超限零正式写入；60 MiB 在 UI 保留和 staging 前双门禁；DB/FS/crash failpoint；提交后 finalization 失败转 Recovery；共享 Recovery latch 与事务中途外部 Recovery；mutation lease 的 actor 重入测试；preview/delete lease；pending/presented/releasing 预览在 UI 与 service 双层拒绝跨附件替换或删除，逐个释放后才能继续；异步预览退出释放 lease；bootstrap-only 恢复；超过 4,096 个合法活动文件；path traversal、symlink、读前 size、bounded read、hash、journal type/phase、无 journal final/trash 反查、所有动态目录与文件的保护和 backup readback；Photos `.current`、非 JPEG 原字节、`FileRepresentation` 实际类型与读前大小门禁；Files 专用 I/O queue；单附件删除不动父记录；Photos/Files 原件不受影响；
- timeline：混合事实、稳定同 instant 顺序、超过 256 同刻/同日 tie 时 fail closed、date-only 路标、effective leaf、长 correction chain 穿越 cursor 的分页无丢失/重复、刷新 supersede 旧分页、当前首刷/分页 corruption 进入 Recovery 且过期 completion 忽略、administration 合并读取 4,096 行预算、合法多 sample 结果容量、关系损坏 fail closed；
